...
function onCreateStudent(name, math, eng, sci) {
  if(!name || !math || !eng || !sci){ //참이면 4개의 입력값중 한개라도 공백 발생
    alert("이름과 모든 과목 점수는 필수 입력사항 입니다.");
    return;
  }

  const avg = ((Number(math) + Number(eng)+Number(sci))/3).toFixed(2);  // .toFixed(2) : 소수점 두자리까지 나타낸다
  ...
  return (
    <div className="App">
      ...
      <StudentList students={students} /> {/* StudentList에 students 저장된값 던져주기위해 */}
